    ...
    def set_kv_buffer(
        ...
        if get_is_capture_mode() and self.alt_stream is not None:
            ...
            if _is_npu and len(loc.size()) > 0 and loc.ndim == 1:
                # The scatter_update operator has certain limitations; please refer to website
                # `https://www.hiascend.com/document/detail/zh/Pytorch/700/apiref/apilist/ptaoplist_000897.html`
                torch_npu.scatter_update_(
                    self.k_buffer[layer_id - self.start_layer].unsqueeze(0),
                    loc[0],
                    cache_k.unsqueeze(0),
                    axis=1,
                )
                torch_npu.scatter_update_(
                    self.v_buffer[layer_id - self.start_layer].unsqueeze(0),
                    loc[0],
                    cache_v.unsqueeze(0),
                    axis=1,
                )
