...
class AscendMLAPagedTokenToKVPool(MLATokenToKVPool):
    ...
    def set_kv_buffer(
        ...
        if layer_id == 0:
            ...
            logger.info(f"set_kv_buffer before {sums[:13]=} {self.kv_buffer[0].sum()=} {self.kv_buffer[0].shape=}")
        ...
        if layer_id == 0:
            sums = self.kv_buffer[0].sum(dim=(1,2))
            logger.info(f"set_kv_buffer after {sums[:13]=} {self.kv_buffer[0].sum()=} {self.kv_buffer[0].shape=}")
